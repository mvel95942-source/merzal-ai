...
export function ShareSheet({ item, onClose }: { item: ShareTarget; onClose: () => void }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: '#1d1a1688', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 60, animation: 'mz-fadein .15s both' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 'min(440px, 100vw)', background: 'var(--paper-panel, #f6f3ec)', borderRadius: '18px 18px 0 0', padding: '18px 18px max(20px, env(safe-area-inset-bottom))', boxShadow: '0 -10px 40px #0003' }}>
        ...
        {url && <div style={{ fontSize: 12, color: 'var(--muted, #7a7166)', background: '#fff', border: '1px solid var(--line-strong, #ddd4c5)', borderRadius: 9, padding: '8px 10px', marginBottom: 14, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{url}</div>}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(72px, 1fr))', gap: 10 }}>
          {targets.map((t) => (
            <button key={t.label} onClick={t.run} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '12px 6px', border: '1px solid var(--line-strong, #ddd4c5)', borderRadius: 12, background: '#fff', cursor: 'pointer' }}>
              <span style={{ fontSize: 22 }}>{t.icon}</span>
              <span style={{ fontSize: 12, color: 'var(--ink, #1d1a16)' }}>{t.label}</span>
            </button>
          ))}
        </div>
        <button onClick={onClose} style={{ width: '100%', marginTop: 14, height: 42, borderRadius: 11, border: '1px solid var(--line-strong, #ddd4c5)', background: '#fff', fontSize: 14, fontWeight: 600 }}>Close</button>
      </div>
    </div>
  )
}
